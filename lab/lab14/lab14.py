...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        return f"Accountholder: {self.holder}, Deposit: {self.deposit_num}, Withdraw: {self.withdraw_num}"
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if self.holder == check.name and check.deposited == False:
            self.deposit(check.value)
            check.deposited = True
            return check.value
        else:
            print(f"The police have been notified.")
            return
...
def filterl(lst, pred):
    ...
    if lst is Link.empty:
        return lst
    return filterl(lst.rest, pred) if not pred(lst.first) \
            else Link(lst.first, filterl(lst.rest, pred))
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lst if lst is Link.empty \
           else foldl(lst, lambda res, first: Link(first, res), Link.empty)
