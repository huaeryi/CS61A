...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        cal = scheme_eval(first, env)
        validate_procedure(cal)

        if isinstance(operator, MacroProcedure):
            print("DEBUG: apply_macro", rest, env)
            return operator.apply_macro(rest, env)

        opp = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(cal, opp, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_eval(expressions.first, env, True) # Tail
    else:
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while tmp is not nil:
            arguments_list.append(tmp.first)
            tmp = tmp.rest
        if self.use_env:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        helper = Pair(formals, body)
        tmp = do_lambda_form(helper, env)
        env.define(name, tmp)
        return name
        # END PROBLEM 9
    else:
...
def do_if_form(expressions, env):
    ...
    if is_true_primitive(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env, True)  # Tail
    elif len(expressions) == 3:
        return scheme_eval(expressions.rest.rest.first, env, True)  # Tai;
...
def make_let_frame(bindings, env):
    ...
    while n is not nil:
        ...
        names = Pair(tmp.first, names)
        values = Pair(scheme_eval(tmp.rest.first, env), values)
        n = n.rest
