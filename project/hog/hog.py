...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    len1 = len(str(player_score))
    if len1 >= 2:
        player_score //= 10 ** (len1 - 2)
    player_first = player_score // 10 % 10
    player_second = player_score % 10

    len2 = len(str(opponent_score))
    if len2 >= 2:
        opponent_score //= 10 ** (len2 - 2)
    opponent_first = opponent_score // 10 % 10
    opponent_second = opponent_score % 10
    return player_first < opponent_first and player_second < opponent_second
    # END PROBLEM 4
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    return 0 if more_boar(score + piggy_points(opponent_score), opponent_score) \
             else piggypoints_strategy(score, opponent_score, cutoff, num_rolls)   # Replace this statement
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return 0 if score + piggy_points(opponent_score) >= GOAL_SCORE \
            else 1 if score + roll_dice(1) >= GOAL_SCORE \
            else 2 if score + roll_dice(2) >= GOAL_SCORE \
            else more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=4)  # Replace this statement
    # END PROBLEM 12
