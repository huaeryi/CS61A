...
def about(topic):
    ...
    def helper(sentence):
        for word in split(sentence):
            for tmp in topic:
                if tmp == lower(remove_punctuation(word)):
                    return True
        return False
    return helper
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    L = []
    for tmp in valid_words:
        if tmp == typed_word:
            return typed_word
        L.append([tmp, diff_function(typed_word, tmp, limit)])
    xmin = min(L, key=lambda x: x[1])
    if xmin[1] > limit:
        return typed_word
    else:
        return xmin[0]
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        ...
        for j in range(len(times_per_player[i]) - 1):
            tmp = times_per_player[i][j + 1] - times_per_player[i][j]
            playertime.append(tmp)
        alltime.append(playertime)
    return game(words, alltime)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    for player in alltimes:
        L.append([])
    for i in range(len(allwords)):
        tmp = [alltimes[x][i] for x in range(len(alltimes))]
        index = tmp.index(min(tmp))
        L[index].append(allwords[i])
    return L
