...
def balanced(m):
    ...
    if (is_planet(m)):
        ...
        if total_weight(end(left(m))) * length(left(m)) == total_weight(end(right(m))) * length(right(m)):
            ...
            tmp = False
        return balanced(end(left(m))) and balanced(end(right(m))) and tmp
...
def has_path(t, word):
    ...
    if len(word) == 1:
        ...
        return word[0] == label(t) and sum([has_path(c, word[1:]) for c in branches(t)]) > 0
...
def check_par():
    ...
    True
    """
    r1 = interval(8, 8)  # Replace this line!
    r2 = interval(6, 6)  # Replace this line!
    return r1, r2


def multiple_references_explanation():
    return """The multiple reference problem...
           Beceuse multiple reference to uncertain number can cause more error"""
...
def quadratic(x, a, b, c):
    ...
    change = -b / a / 2
    if change >= lower_bound(x) and change <= upper_bound(x):
        lower = min(lower, f(change))
        upper = max(upper, f(change))
    return interval(lower, upper)
